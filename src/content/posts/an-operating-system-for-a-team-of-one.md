...
When work touches that list, the agent stops and says so. The agents followed this boundary only after I wrote it down, and it is the rule that has prevented the most damage: unreviewed changes to live infrastructure, a scope quietly doubled, a settled decision reversed in passing.
...
The rules also improve the agents' output, because each reply has to decide what matters before it is written. The one-thing-at-a-time rule governs questions and decisions, not execution: agents can and do work in parallel, each on a narrow packet, with an orchestrator holding the wider context.
